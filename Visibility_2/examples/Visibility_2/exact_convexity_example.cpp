...
private:
    ...
    Kernel::FT calc(const Corner &c1, const Corner &c2, const Direction_2 &newDir) const {


        auto dia = std::min(diagonal.direction(), diagonal.opposite().direction());


        Kernel::FT t = dia.dy() == 0 ? 0 : atan(dia.dy() / dia.dx());
        Kernel::Aff_transformation_2 rotate(CGAL::ROTATION, sin(-t), cos(-t));

//        Kernel::Aff_transformation_2 rotate(CGAL::ROTATION, dia, 1, 1e10);
        cout << "ROTATE: " << dia << ", val" << endl;
        ...
        Kernel::Aff_transformation_2 trans(CGAL::TRANSLATION, Kernel::Vector_2(-PL.x(), 0));
        ...
        Kernel::FT PLX = PL.x() - diff;
        Kernel::FT PLY = PL.y();
        Kernel::FT PRX = PR.x() - diff;
        Kernel::FT PRY = PR.y();

        Kernel::FT TOP0X = TOP0.x() - diff;
        Kernel::FT TOP0Y = TOP0.y();
        Kernel::FT TOP1X = TOP1.x() - diff;
        Kernel::FT TOP1Y = TOP1.y();

        Kernel::FT BOT0X = BOT0.x() - diff;
        Kernel::FT BOT0Y = BOT0.y();
        Kernel::FT BOT1X = BOT1.x() - diff;
        Kernel::FT BOT1Y = BOT1.y();
        ...
        Kernel::FT epsilon = 1e-10;
        Kernel::FT intRes = 0;

        intRes = Integral<Kernel::FT>::ComputeOpt(TOP0X, TOP0Y,
                                                  TOP1X, TOP1Y,
                                                  BOT0X, BOT0Y,
                                                  BOT1X, BOT1Y, 0,
                                                  PLY,
    ...
    static bool is_edge_in_faces(const vector<CTP::Face_handle> &faces, const Segment_2 &seg) {
...
private:
    ...
    Segment_2 find_diagonal(BinaryTree<CTP>::Node *leftNode, BinaryTree<CTP>::Node *rightNode) const {
        int res;
        for (const auto &leftFace: leftNode->data) {
            for (const auto &rightFace: rightNode->data) {
                if (leftFace->has_neighbor(rightFace, res)) {
                    return Segment_2(triangulation.segment(CTP::Edge(leftFace, res)));
                }
            }
        }
    ...
    void find_diagonal_subsets(BinaryTree<CTP>::Node *leftNode, BinaryTree<CTP>::Node *rightNode,
    ...
    Kernel::FT create_event_list(BinaryTree<CTP>::Node *leftNode, BinaryTree<CTP>::Node *rightNode) {
        ...
        for (auto crossingDiagonal = crossingDiagonals.begin();
            ...
            Corner newCorner = {crossingDiagonal->target(), diagMap[*crossingDiagonal],
                                diagMap[crossingDiagonal->opposite()],
            ...
            cout << *crossingDiagonal << endl;
            if (target_in_list == eventList.end()) {
                cout << "APPEARANCE target: " << newCorner.target << endl;
                ...
                cout << "APPEARANCE source: " << newCorner.target << endl;
                ...
                cout << "DISAPPEARANCE target: " << crossingDiagonal->target() << endl;
                cout << "Last placed" << lastPlaced->target << endl;
                cout << "Target in: " << target_in_list->target << endl;
                ...
                cout << "DISAPPEARANCE source: " << crossingDiagonal->source() << endl;
                cout << "Last placed" << lastPlaced->target << endl;

                cout << "Target in: " << source_in_list->target << endl;
